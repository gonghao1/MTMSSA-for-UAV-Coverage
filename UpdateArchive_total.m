function [Archive_X_updated, Archive_F_updated, Archive_F1_updated, Archive_member_no]=UpdateArchive_total(Archive_X, Archive_F, Archive_F1,Particles_X, Particles_F, Particles_F1, Archive_member_no)
...
% Archive_F1_updated = Archive_F1_temp(~isDominated, :);
% % A(any(isnan(A), 2), :) = [];
% 
...
% findinf=unique([findinf' findnan]);
% if ~isempty(findinf)
% Archive_X_updated(findinf,:)=[];
% Archive_F_updated(findinf,:)=[];
% Archive_F1_updated(findinf,:)=[];
% end
...
% Archive_F1_updated = Archive_F1_updated(re_frepeat,:);
% Archive_member_no=size(Archive_F_updated,1);
 ...
 re_frepeat=[];
Archive_X_temp=[Archive_X; Particles_X'];% 维度由种群数量和archive  300x5
Archive_F_temp=real([Archive_F; Particles_F]);% 300x2
Archive_F1_temp=[Archive_F1; Particles_F1];
% isDominated = false(size(Archive_F_temp, 1), 1);
% for i = 1:size(Archive_F_temp, 1)
%     for j = i+1:size(Archive_F_temp, 1)
